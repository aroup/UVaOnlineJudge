...
map<point,int> MC,MR;
bool inPolygon(point pt,const vector<point> &P,map<point,int> M)
{
    ...
    REP(i,0,P.size()-2)
    {
        if(distToLineSegment(pt,P[i],P[i+1],q)<EPS&&(!(q==P[i])&&!(q==P[i+1])))
        {
            return true;
        }
    }
